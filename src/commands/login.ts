...
export async function loginCommand(opts: { json?: boolean }): Promise<void> {
  ...
  try {
    ...
    if (opts.json) {
      console.log(
        JSON.stringify(
          { ok: true, displayName: result.displayName, expiresAt, hasExplicitExpiry: result.cookieMeta.hasExplicitExpiry },
          null,
          2
        )
      );
      ...
      if (expiresAt) {
        ...
        console.log(
          "The server did not set an explicit session expiry (no Max-Age/Expires on the cookie). " +
